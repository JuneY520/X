/*
 * @Author: Sliverkiss
 * @Date: 2023-09-16 21:51:27
 * @FilePath: https://github.com/Sliverkiss/helloworld/Study/bym.js
 * @Description:
 * 微信小程序 朵茜情调生活馆 每日签到 连续签到送情趣玩具，如跳蛋等
 * 捉h5.youzan.com域名任意包下的Cookie中的KDTWEAPPSESSIONID=XXXXXXXXXX,填写到dxqd_cookie中,多帐号换行
 * 
...
function checkin(user) {
    return new Promise((resolve) => {
        ...
        const signinRequest = {
            url: "https://h5.youzan.com/wscump/checkin/checkinV2.json?checkinId=3262",
            headers: header,
        };
...
//用户状态接口
async function getCustomerPoints(user) {
    return new Promise((resolve) => {
        const header = {
            Cookie: user.cookie,
            "User-Agent": " Mozilla/5.0 (iPhone; CPU iPhone OS 14_8 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.37(0x18002524) NetType/WIFI Language/zh_CN miniProgram/wx6b30ed1839d47d45"
        ...
        $.get(signinRequest, (error, response, data) => {
            ...
            var result = JSON.parse(body);
            if (result?.code == 0) {
                message += `\n帐号[${user.index}]${result?.data.userId} 余额：${result?.data?.totalAmount}`;
            } else {
