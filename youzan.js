...
获取ck方法:使用Stream等抓包软件，抓取你想要签到的有赞小程序中h5.youzan.com域名任意包下的Extra-data中的sid=XXXXXXXXXX,按照格式要求填写到youzan_data中,多个任务用,隔开

具体格式要求如下:
{"签到任务编号":"sid","签到任务编号":"sid","签到任务编号":"sid",}

举例：如果你只想签到参半口腔护理和伯喜线上商城，可以如下填写：
{"cbkqhl":"XXXXXXXX","bosie":"XXXXXXXXX"}

脚本兼容：Surge、QuantumultX、Loon、Shadowrocket、Node.js
------------------------------------------
签到任务编号
------------------------------------------
- 参半口腔护理:cbkqhl =>地址:#小程序://参半口腔护理/VUmyyDiSXKtfznw
- 伯喜线上商城:bosie =>地址:#小程序://bosie伯喜线上商城/JIqNBdpsvG6npZI
- AchockHouse:AchockHouse =>地址:#小程序://AchockHouse/MvCf1r6E7CsrFUh
- 海贽医疗科技:hzylkj =>地址:#小程序://海贽医疗科技/EOUypu2eGQrnTaD
- INTOYOU心慕与你:intoyou =>地址:#小程序://INTOYOU/liTL0FueXf3pHAs
- 蜜蜂惊喜社:mfjxs =>地址:#小程序://蜜蜂惊喜社/v4EAxUHy4pEs9Pu
- 红之旗舰店:hzqjd =>地址:#小程序://兰蔻官网精品商城/YUzkN1UEJPzbEKy
- 花洛莉亚:flortte =>地址:#小程序://FLORTTE/Nxc2W1tujMU42Dh
- 且初:kimtrue =>地址:#小程序://KIMTRUE且初/Fgl4s4NXaGVLjSm
- 菲诗蔻官方商城:ficcecode =>地址:#小程序://FicceCode菲诗蔻官方商城/3hXLYsLDpk0MP0e
- 肤漾:foryon =>地址:#小程序://肤漾FORYON/6aH9H00dfxxGWno
- 且悠:chillmore =>地址:#小程序://chillmore且悠/mPN6nTLvQlW3SLj
- HBN研究所:hbn =>地址:#小程序://HBN颜究所/4oxWftaFiyWcF6z
- 珂拉琪旗舰店:colorkey =>地址:#小程序://珂拉琪/tAiO5gNQALlcEDe
- 爱依服有赞商城:aiyifu =>地址:#小程序://爱依服商城/6zUc4YrApA3HWHa
- 莱克旗舰店:lkqjd =>地址:#小程序://莱克旗舰店/uhVa5C07KIxq73w
- PMPM:pmpm =>地址:#小程序://PMPM/6Ad2juXgIBQNiCf
- LAN蘭:lan =>地址:#小程序://LAN蘭/SK3ihWIMyt8uo2x
- 云南白药生活+:ynbyshj =>地址:##小程序://云南白药生活+/iPUDMyGwSt4TeTl
- ffit8会员俱乐部:ffit
- 朵茜情调生活馆:dxqd
- xbox聚乐部:xbox
- 三只松鼠:szss
...
Surge:
[Script]
Sakura Checkbox = type=cron,cronexp=30 8 * * *,timeout=300,script-path=https://gist.githubusercontent.com/Sliverkiss/585daf69a0a6c7243db8993f5394a124/raw/youzan.js,script-update-interval=0
------------------------------------------
⚠️【免责声明】
------------------------------------------
...
async function main() {
    let accountList = await getAccount();
    if (accountList) {
        let todoList = [];
        //数据过滤，只留下要执行的任务
        accountList.forEach(item => checkList.includes(item.key) && todoList.push(item));
        debug(todoList, "过滤数据");
        //打广告
        ...
        for (let item of todoList) {
            //获取ck
            item.token = userCookie[`${item.key}`]
            //执行签到任务
...
async function signin(item) {
    try {
        const options = {
            //签到任务调用签到接口
            url: `https://h5.youzan.com/wscump/checkin/checkinV2.json?checkinId=${item.checkinId}&app_id=${item.appId}&kdt_id=${item.kid}`,
            //请求头, 所有接口通用
            headers: {
                "Extra-Data": "{ \"is_weapp\": 1,\"sid\":\"" + item.token + "\" }",
            ...
            $.get(options, async (error, response, data) => {
                try {
                    ...
                    if (result?.code == 0) {
                        ...
                        DoubleLog(`【${item.name}】 >> ❌Check ck error!`)
...
async function point(msg, item) {
    try {
        const options = {
            //签到任务调用签到接口
            url: `https://h5.youzan.com/wscump/pointstore/getCustomerPoints.json?app_id=${item.appId}&kdt_id=${item.kid}`,
            //请求头, 所有接口通用
            headers: {
                "Extra-Data": "{ \"is_weapp\": 1,\"sid\":\"" + item.token + "\" }",
                "User-Agent": " Mozilla/5.0 (iPhone; CPU iPhone OS 14_8 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.37(0x18002524) NetType/WIFI Language/zh_CN miniProgram/wx6b30ed1839d47d45",
        ...
        return new Promise(resolve => {
            $.get(options, async (error, response, data) => {
                try {
                    let result = JSON.parse(data);
                    DoubleLog(`【${item.name}】${msg},积分：${result?.data?.currentAmount}`)
                } catch (error) {
                    $.log(`❌出现错误！原因为:${error}`);
                } finally {
                    resolve();
...
async function getAccount() {
    try {
        const options = {
            //签到任务调用签到接口
            url: `https://gist.githubusercontent.com/Sliverkiss/390bd056cfc194bca47ecf8adcb01209/raw/youzan.json`,
            //请求头, 所有接口通用
            headers: {
                "User-Agent": ""
            }
